function [sys,x0,str,ts] = competition_flyvep3(t,x,u,flag,Host,Socket,...
...
switch flag,
    case 0
        ...
        load ('D:\liuqianqian\s1\modetest_off_static','CRF')%运行起来需要自己先输入一组随机的模型数据，然后一轮后加入自己的数据
        gUDPcloseALL;
%         pnet('closeall');
...
%          end
        ...
        elseif rtBCI.mfp300_without_fb.starget==4
         ...
         elseif rtBCI.mfp300_without_fb.starget==5     
               rtBCI.mfp300_without_fb.Letters=ones(1,100)+90;
    ...
    case 3
        ...
        if rtBCI.count2==0
            ...
            if rtBCI.mfp300_without_fb.wait*3==rtBCI.mfp300_without_fb.waitCounter
...
%                     display(num2str(rtBCI.mfp300_without_fb.out));
...
%                     rtBCI.mfp300_without_fb.nrByte=gUDPsend(rtBCI.mfp300_without_fb.h1,rtBCI.host2,Socket,'4 ');
            ...
            elseif(TRUE==rtBCI.mfp300_without_fb.ResetSpeller)
                rtBCI.mfp300_without_fb.PauseCounter = rtBCI.mfp300_without_fb.PauseCounter+1;
                if(rtBCI.mfp300_without_fb.PauseCounter>255)
                    rtBCI.mfp300_without_fb.PauseCounter=0;
                rtBCI.mfp300_without_fb.ResetSpeller=FALSE;
                end
            ...
            elseif((FALSE == rtBCI.mfp300_without_fb.Pause) && (FALSE == rtBCI.mfp300_without_fb.ResetSpeller))
                ...
                if rtBCI.mfp300_without_fb.state==0&&rtBCI.mfp300_without_fb.label==1
                    if rtBCI.mfp300_without_fb.stage1==1
                        ...
                        rtBCI.mfp300_without_fb.nrByte=gUDPsend(rtBCI.h1,rtBCI.host1,rtBCI.socket1,num2str(rtBCI.mfp300_without_fb.indices(rtBCI.mfp300_without_fb.indicesCnt)+100));
...
enlab=eleNum-1;
seableNum=4;
indord=ones(1,eleNum)*18;
t=1;
if lastone==0;
    randfir=randperm(eleNum);%get randon order
    a1=randfir(1)-1;
end
if lastone>0
    if lastone==NumCustomCmds
        tx=randperm(4);
        if tx(1)==1
            a1=1;
        elseif tx(1)==2
    ...
    elseif lastone==NumCustomCmds-1
        tx=randperm(4);
        if tx(1)==1
            a1=1;
        elseif tx(1)==2
